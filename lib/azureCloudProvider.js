        ...
        .then((credentials) => {
            ...
            if (credentialsJson.storageAccount && credentialsJson.storageKey) {
                this.storageClient = azureStorage.createBlobService(
                   'DefaultEndpointsProtocol=https;AccountName=' + credentialsJson.storageAccount + ';AccountKey=' + credentialsJson.storageKey + ';EndpointSuffix=core.chinacloudapi.cn;' 
                );
...
AzureCloudProvider.prototype.getDataFromUri = function getDataFromUri(uri) {
    const azureRegex = /https:\/\/[a-z0-9]+\.blob\.core\.chinacloudapi\.cn/;
    ...
    let parts = uri.split('blob.core.chinacloudapi.cn/');

    // Get container and blob
    parts = parts[1].split('/');
    if (parts.length < 2) {
        const exampleURI = 'https://account.blob.core.chinacloudapi.cn/container/blob';
